  ...
  return (
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 space-y-4 max-w-md mx-auto">
      ...
      <div className="flex justify-center gap-2 mt-2 flex-wrap">
        {!recording ? (
          <button onClick={startRecording} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded">
            Grabar
          </button>
        ) : (
          <button onClick={stopRecording} className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded">
            Detener
          </button>
        )}
        <button onClick={prevStep} className="bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded">
          Atrás
        </button>
        <button onClick={nextStep} disabled={!videoBlob} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded">
          Siguiente
        </button>
      </div>
      ...
      {videoBlob && (
        <video src={URL.createObjectURL(videoBlob)} controls playsInline className="w-48 h-48 mt-2 mx-auto rounded-lg border" />
      )}
    </div>
  );
}
